...
function SearchBar() {
    ...
    const handleInputChange = (event: { target: { value: SetStateAction<string>; }; }) => {
        ...
        if (val != '') {
            let filteredQuestions = questions.filter((obj) => {
            return (obj?.question.toLowerCase().replace('and', '&').includes(val.toLowerCase().replace('and', '&')) || obj?.category.toLowerCase().replace('and', '&').includes(val.toLowerCase().replace('and', '&')))})
        } else {
            this.searchResult = []
        }
    };
